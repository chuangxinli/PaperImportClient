<template>
  <div>
    ...
    <el-button type="primary" @click="confirmImport" v-show="!isUploading">点击上传</el-button>
                <el-button type="primary" v-show="isUploading">正在上传中。。。</el-button>
  </span>
    </el-dialog>
...
<script>
  ...
  function dealExaminationPoints(primaryStr, jsonObj, subjectAboutInfo) {
    let tempArr = primaryStr.replace(dealBracket('考点'), '').replace(/\s*/g, '').replace(/(．\S*|\.\S*|。\S*)/, '').replace(/:/g, '：').replace(/;/g, '；').split('；'),
      ExaminationPoints = []
  ...
  export default {
    ...
    methods: {
      htmlToJson(originArr, myEmitter, subjectAboutInfo, unitAndSubUnit) {
        let docxArr = []
        for (let i = 0, len = originArr.length; i < len; i++) {
          docxArr.push(originArr[i].path)
        }
      ...
      confirmImport() {
        ...
        //ipcRenderer.send('docxList', this.docxList)
        /*ipcRenderer.once('reply', (e, data) => {
        ...
        this.htmlToJson(this.docxList, myEmitter, this.subjectAboutInfo, this.unitAndSubUnit)
      },
      selectFile() {
